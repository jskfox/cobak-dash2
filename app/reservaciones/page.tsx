...
export default function ReservacionesPage() {
  ...
  const EventComponent = ({ event }) => (
    <div className="flex items-center p-1 bg-red-100 dark:bg-red-900 rounded-md shadow-sm">
      <img src={event.client.avatar} alt={event.client.name} className="w-6 h-6 rounded-full mr-2" />
      <div>
        <div className="font-bold text-sm">{event.title}</div>
        <div className="text-xs text-gray-600 dark:text-gray-300">{event.resourceId}</div>
      </div>
    </div>
  );
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-red-50 to-orange-100 dark:from-gray-900 dark:to-red-900 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-red-700 dark:text-red-300">Calendario de Reservaciones</h1>
      <div className="flex justify-between mb-6">
        ...
        <Dialog>
          <DialogTrigger asChild>
            <Button className="bg-red-600 hover:bg-red-700 text-white">Agregar Reservación</Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Nueva Reservación</DialogTitle>
            </DialogHeader>
            <ReservationForm onSubmit={handleAddReservation} clients={mockClients} rooms={rooms} />
          </DialogContent>
        </Dialog>
      </div>
